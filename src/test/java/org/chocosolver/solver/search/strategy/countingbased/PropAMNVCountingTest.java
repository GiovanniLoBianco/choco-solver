...
public class PropAMNVCountingTest {

	public static CountingTools tools = new CountingTools();

	public static PropAMNV createInstance1() {
		...
		IntVar N = model.intVar(3);

		Constraint c = model.atMostNValues(vars, N, true);
		c.post();
		return (PropAMNV) c.getPropagator(1);
	}
	
	public static PropAMNV createInstance2() {
		Model model = new Model();
		IntVar[] vars = new IntVar[7];
		vars[0] = model.intVar(new int[] { 2, 4, 6 });
		vars[1] = model.intVar(new int[] { 3, 4, 9 });
		...
		Constraint c = model.nValues(vars, N);
		c.post();
		return (PropAMNV) c.getPropagator(2);
	}
