...
public class CarSequencing extends AbstractProblem {
	...
	public void buildModel() {
		...
		int[] numClasses = new int[nClasses];
		...
		for (int optNum = 0; optNum < options.length; optNum++) {
			...
			for (int seqStart = 0; seqStart <= (cars.length - optfreq[optNum][1]); seqStart++) {
				...
				IntVar[] atMost = new IntVar[nClasses];
				int countOptions = 0;
				IntVar[] constrainedClasses = new IntVar[nbConf];
				for (int i = 0; i < nClasses; i++) {
					// optfreq[optNum][0] times AT MOST

					int k = 0;
					while (k < nbConf && options[optNum][k] != i) {
						k++;
					}
					if (k == nbConf) {
						atMost[i] = model.intVar("atmost_" + optNum + "_" + seqStart + "_" + nbConf, 0,
								optfreq[optNum][1], true);
					} else {
						atMost[i] = model.intVar("atmost_" + optNum + "_" + seqStart + "_" + nbConf, 0,
								optfreq[optNum][0], true);
						constrainedClasses[countOptions] = atMost[i];
						countOptions++;
					}
				}

				model.globalCardinality(carSequence, numClasses, atMost, false).post();

				// all others configurations may be chosen
				IntVar sum = model.intVar("sum", 0, optfreq[optNum][0], true);
				model.sum(constrainedClasses, "=", sum).post();
			}
		}

		int[] values = new int[expArray.length];
		for (int i = 0; i < expArray.length; i++) {
			expArray[i] = model.intVar("var_" + i, demands[i], demands[i], false);
			values[i] = i;
		}
		model.globalCardinality(cars, values, expArray, false).post();

		// First deduction rule
		model.sum(expArray, "=", cars.length).post();

		// Second deduction rule
		int sumUBVars = 0;
		for (int i = 0; i < cars.length; i++) {
			sumUBVars +=cars[i].getUB();
		}
		IntVar scalar = model.intVar(0, sumUBVars);
		model.scalar(expArray, values, "=", scalar).post();
		model.sum(cars, "=", scalar);
